...
function askPriceStep(ctx: FlowContext, productId: string): Step {
  ...
  return {
    kind: 'quinn',
    text: `Current pack cost is **${cur.toFixed(2)} DH**. Pick a new price or pick a percentage adjustment.`,
    options: [
      { label: `${(cur * 0.95).toFixed(2)} DH (-5%)`, next: () => previewStep(setProductPrice(ctx.products, productId, +(cur * 0.95).toFixed(2)), () => startProductScopedFlow(ctx, productId)) },
      { label: `${(cur * 1.05).toFixed(2)} DH (+5%)`, next: () => previewStep(setProductPrice(ctx.products, productId, +(cur * 1.05).toFixed(2)), () => startProductScopedFlow(ctx, productId)) },
      { label: `${(cur * 1.1).toFixed(2)} DH (+10%)`, next: () => previewStep(setProductPrice(ctx.products, productId, +(cur * 1.1).toFixed(2)), () => startProductScopedFlow(ctx, productId)) },
