...
import { MAX_BENCHES, useSiteSettingsStore } from '@/components/Settings/siteSettingsStore';
...
import { FJ_WORK_ROLES, FJ_WORK_ROLE_BY_ID } from './fjFixtures';
...
import { channelsText, classLabel, diffDays, diffRecipeFields, groupLabel, diffStations, kitSummary, rolesText, stationsToOverlay, toStationDraft, withCompanyDays, withShopDays, type DaysDraft, type ShopDays, type StationDraft } from './setupModel';
...
export default function SetupScreen({ initialTab }: { initialTab?: string } = {}) {
  const { isFarmerJ } = useActiveSite();
  const store = useFjPlanStore();
  const [tab, setTab] = useState<Tab>(TABS.some(t => t.id === initialTab) ? (initialTab as Tab) : 'recipes');
...
function diffRecipes(recipes: Recipe[], draft: RecipeDraft): SettingsChange[] {
  return recipes.flatMap(r => (draft[r.id] ? diffRecipeFields(r.name, productionFieldsOf(r), fieldsWithDraft(r, draft)) : []));
}
            ...
            <div key={e.id || i} style={{ padding: '12px 14px', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-item)', background: i === 0 ? 'var(--color-bg-hover)' : '#fff', opacity: reverted ? 0.7 : 1 }}>
              ...
              {e.said && (
                <div style={{ marginTop: 6, fontSize: 12, color: 'var(--color-text-secondary)' }}>
                  <span style={{ fontStyle: 'italic' }}>“{e.said}”</span>
                  <span style={{ color: 'var(--color-text-muted)' }}> · asked in the Command Centre</span>
                </div>
              )}
