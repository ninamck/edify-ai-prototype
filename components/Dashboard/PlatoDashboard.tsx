...
import FlowMetRecipeTab from '@/components/Dashboard/Plato/FlowMetRecipeTab';
import FlowDaySelector from '@/components/Dashboard/Plato/FlowDaySelector';
import StoreSelector from '@/components/Dashboard/Plato/StoreSelector';
import { FLOW_BASE_DATE, PLATO_STORES } from '@/components/Dashboard/data/platoMockData';
import StoreCheckTab from '@/components/Dashboard/Plato/StoreCheckTab';
import BaristaEvaluationTab from '@/components/Dashboard/Plato/BaristaEvaluationTab';
...
type PlatoTab = 'flow' | 'storecheck' | 'barista';

const TABS: { id: PlatoTab; label: string }[] = [
  { id: 'flow', label: 'Flow · Met Recipe' },
  { id: 'storecheck', label: 'Store check' },
  { id: 'barista', label: 'Barista evaluation' },
];

export default function PlatoDashboard({
  layout,
  editing,
  onLayoutChange,
  onToggleEdit,
  ...
  const [tab, setTab] = useState<PlatoTab>('flow');
  // Selected day and store — kept here so they survive tab switches.
  const [flowDate, setFlowDate] = useState(FLOW_BASE_DATE);
  const [store, setStore] = useState(PLATO_STORES[0]);
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ minWidth: 0 }}>
          {heroGreeting ? (
            <>
              ...
              >
                Flow telemetry, store checks and barista evaluations in one place
              </p>
            </>
          ) : (
            <>
              <h1 style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 700, color: 'var(--color-text-primary)' }}>
                Platō Coffee <span style={{ color: 'var(--color-text-muted)', fontWeight: 600 }}>· quality dashboard</span>
              </h1>
        ...
        <DashboardEditToolbar
          ...
          leadingControls={
            <>
              {toolbarLeadingControls}
              <StoreSelector store={store} onChange={setStore} />
              <FlowDaySelector date={flowDate} onChange={setFlowDate} />
            </>
          }
        />
      </div>
      ...
      <div
        role="tablist"
        aria-label="Platō Coffee dashboard view"
        style={{
      ...
      <div role="tabpanel">
        {tab === 'flow' && <FlowMetRecipeTab date={flowDate} store={store} />}
        {tab === 'storecheck' && <StoreCheckTab store={store} />}
        {tab === 'barista' && <BaristaEvaluationTab store={store} />}
      </div>
