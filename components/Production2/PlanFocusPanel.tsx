...
const palettes: Record<FocusReason, { bg: string; border: string; icon: string }> = {
  ...
  // PAC147 — error-tinted because by the time we surface it the floor is
  // either already out or about to run out within the threshold window.
...
function buildAffected(lines: PlanLine[], focusedItemId: string, reason: FocusReason, lowStockMap: LowStockMap): PlanLine[] {
  ...
  switch (reason) {
    ...
    case 'draft-forecast':
      ...
      break;
    case 'lowstock':
      // Use the snapshot map the panel pre-built. Falls back to an empty
      // pool when the host didn't provide sold-counts (lowstock requires
      // them; defensive only).
      pool = lowStockMap
  ...
  switch (reason) {
    ...
    case 'lowstock': {
      ...
      const soldOuts = lowStockMap
        ...
        : 0;
      // Tone of the headline tracks the live state — sold-out reads
      // worse than running-low and we want the body copy to match.
...
function cardDetail(line: PlanLine, reason: FocusReason, lowStockMap: LowStockMap): {
  ...
  switch (reason) {
    ...
    case 'lowstock': {
      ...
      // Suggested top-up: cover the threshold buffer plus a small head-room
      // so the next push has slack. Sold-out rows get the gap + buffer; rows
      // still inside the threshold get the difference back to a healthy line.
...
/** Shared topup math between the action label and the action handler so
 *  the button label always matches what gets written. Mirrors the
 *  `suggestedTopUp` calculation in `cardDetail`. */
...
function applyPrimaryAction(
  ...
  switch (reason) {
    ...
    case 'draft-forecast':
      ...
      return;
    case 'lowstock': {
      // Bump the day total by the suggested top-up. setPlanned writes the
      // whole-day number, which feeds runPlanned + variablePlanned downstream
      // — the kitchen sees the update as additional variable production
      // because the run baselines are already locked at this point in the
      // day (low-stock fires after 11:00).
      const top = suggestedLowStockTopUp(line, lowStockMap);
...
function isResolved(line: PlanLine, reason: FocusReason, lowStockMap: LowStockMap): boolean {
  switch (reason) {
    ...
    case 'lowstock': {
      // Resolved when the row is no longer in the snapshot map — i.e. the
      // remaining buffer is back above the threshold (because the manager
      // bumped the plan).
